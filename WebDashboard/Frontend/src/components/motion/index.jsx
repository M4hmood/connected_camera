    ...
    return (
        <div>
          ...
          <div style={{display: 'flex', width: '300px', 'justify-content': 'space-evenly', 'align-items': 'flex-start'}}>
            <p style={{ color: motionDetected ? 'blue': 'gray'}}>{motionDetected ? 'Motion Detected' : 'No Motion Detected'}</p>
            {motionDetected ? <DirectionsRunIcon style={{color: 'red'}}/> : <AccessibilityIcon style={{color: 'aquamarine'}}/>}
          </div>
        </div>
    );
};
